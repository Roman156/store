document.addEventListener('DOMContentLoaded', () => {
	...
	const showThankYouModal = () => {
		localStorage.removeItem('products');
		alert('Спасибо за покупку!'); // Пример с использованием alert
	};

	function validateForm() {
		var name = document.forms["order"]["Имя"].value;
		var phone = document.forms["order"]["Телефон"].value;
	...
	document.addEventListener('submit', () => {
		let flag_v = validateForm();
		if (flag_v) {
			showThankYouModal(); // Показываем сообщение "Спасибо за покупку"
			location.reload()
		}
	});
});
